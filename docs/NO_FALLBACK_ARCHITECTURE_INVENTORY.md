...
| NF-005 | Lokale Persistenz | `apps/brain-app/src/viewer/atlas/atlasConfigStore.ts`, `apps/brain-app/src/viewer/settingsStore.ts`, `apps/brain-app/src/viewer/authoringSnapshotStore.ts`, `apps/brain-app/src/viewer/localDataActions.ts`, `docs/ARCHITECTURE.md` | [Entscheide localStorage-Override-Architektur und Persistenzgrenzen](https://app.dartai.com/t/1Hbpo13ABwto-Entscheide-localStorage-Overri) | Jeden `localStorage`-Key einem aktuellen UI-Zweck oder Löschpfad zuordnen. Keine kanonische Config, kein Lernfortschritt und keine Migrationsbrücke dürfen still lokal hängen. | `pnpm --dir apps/brain-app test -- src/viewer/atlas/atlasConfigStore.test.ts src/viewer/settingsStore.test.ts src/viewer/authoringSnapshotStore.test.ts src/viewer/localDataActions.test.ts` |
...
| NF-008 | Aktuelle Doku-Drift | `README.md`, `docs/ARCHITECTURE.md`, `docs/MASTERPLAN.md`, `docs/ASSET_UND_INHALTSINVENTUR.md`, `docs/SP5_1_CANONICAL_LINKS.md`, `docs/FAERBUNGEN_GUIDE.md` | [Bereinige aktuelle Doku-Drift zu Legacy/Fallback/Architekturstatus](https://app.dartai.com/t/n0RsZZ5gXo5X-Bereinige-aktuelle-Doku-Drift) | Aktuelle Arbeitsdoku an Code-Endzustand angleichen. Historische Reviews/End-Session-Dateien nicht umschreiben, aber nicht als Arbeitsanweisung nutzen. | `rg -n "legacy|fallback|deprecated|localStorage" README.md docs --glob '!docs/reviews/**' --glob '!docs/END_SESSION_*' --glob '!docs/superpowers/plans/**'` |
...
| NF-012 | Parser-/Default-Fallbacks | `apps/brain-app/src/viewer/settingsStore.ts`, `apps/brain-app/src/viewer/viewerStateSnapshot.ts`, `apps/brain-app/src/viewer/contractValidation.ts`, `apps/brain-app/src/viewer/manifestAuthoringRuntime.ts` | [Kläre Parser-/Default-Fallbacks in Settings, Snapshots und Authoring](https://app.dartai.com/t/67dIBiXChW0p-Kl-re-Parser-Default-Fallbacks) | Schema-Defaults, Migration und Fehlerpfade trennen. Korrupte oder alte Daten dürfen nicht still als gültige App-Wahrheit repariert werden. | `pnpm --dir apps/brain-app test -- src/viewer/settingsStore.test.ts src/viewer/viewerStateSnapshot.test.ts src/viewer/contractValidation.test.ts src/viewer/localDataActions.test.ts` |
