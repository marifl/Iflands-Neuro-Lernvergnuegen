...
| --- | --- | --- | --- | --- | --- |
| NF-001 | Kamera-Legacy-Fallback | `apps/brain-app/src/scene/cameraRigConfig.ts`, `apps/brain-app/src/scene/CameraRig.tsx`, `apps/brain-app/src/scene/cameraResolve.ts`, `apps/brain-app/src/scene/cameraRigConfig.test.ts`, `apps/brain-app/src/scene/cameraResolve.test.ts`, `apps/brain-app/src/scene/CameraRig.test.tsx` | [Entscheide und migriere Kamera-Legacy-Fallback](https://app.dartai.com/t/vACjRDOH2EFc-Entscheide-und-migriere-Kamera) | Migriert am 19. Juni 2026: URL-/Configuration-Kameras und Scene-Kameras sind die einzigen aktuellen Quellen. `sceneStore.cameraShot` wird als explizite Scene-Kamera übersetzt; der Selector kennt keine `legacy`-Source mehr und der Resolver akzeptiert keinen Shot-Fallback mehr. | `pnpm --dir apps/brain-app exec vitest run src/scene/cameraRigConfig.test.ts src/scene/cameraResolve.test.ts src/scene/CameraRig.test.tsx`; `pnpm --dir apps/brain-app typecheck`; `rg -n -e "legacyCameraConfig" -e "fallbackShot" -e "CameraConfigSource = 'legacy'" apps/brain-app/src` |
| NF-002 | Animation-Legacy | `apps/brain-app/src/viewer/animations.ts`, `apps/brain-app/src/viewer/animationSystem.ts`, `apps/brain-app/src/viewer/animationSystem.test.ts`, `apps/brain-app/src/viewer/contractValidation.ts`, `apps/brain-app/src/viewer/AnimationPlayer.tsx`, `apps/brain-app/src/scene/overlays/OverlayPanel.tsx` | [Kläre AnimationPlayer- und legacyAnimations-Migration](https://app.dartai.com/t/VbYqUgAngZE1-Kl-re-AnimationPlayer-und) | Migriert am 19. Juni 2026: Die Basalganglien-Schleife ist ein registriertes `TimelineDocument`; der Player liest die Timeline direkt, ContractValidation nutzt registrierte Timeline-Bindings plus AuthoringScene-ClipBindings, und die alten Adapterbegriffe sind aus aktuellem `src/viewer`-/`src/scene`-Code entfernt. | `pnpm --dir apps/brain-app exec vitest run src/viewer/animationSystem.test.ts src/viewer/contractValidation.test.ts src/scene/overlays/OverlayPanel.test.tsx`; `rg -n "legacyAnimations\|legacy-highlight\|legacy:" apps/brain-app/src/viewer apps/brain-app/src/scene` |
| NF-003 | Mesh-Picking-Legacy | `apps/brain-app/src/viewer/targetPicking.ts`, `apps/brain-app/src/viewer/CutPickBridge.tsx`, `apps/brain-app/src/viewer/viewerStore.ts`, `apps/brain-app/src/viewer/BodyParts3DViewer.tsx`, `apps/brain-app/src/viewer/targetPicking.test.ts`, `apps/brain-app/src/viewer/viewerStore.test.ts`, `apps/brain-app/src/viewer/cutPickTargets.test.ts`, `apps/brain-app/src/viewer/cutPick.test.ts` | [Kläre Legacy-Mesh-Picking-Fallback](https://app.dartai.com/t/E7CfpLJUiG6R-Kl-re-Legacy-Mesh-Picking) | Migriert am 19. Juni 2026: Ontologie-Meshnamen sind ein aktueller TargetRef-Vertrag. Brain-/Kontext-Meshes erhalten `sequenceTargetUserData`; `CutPickBridge` löst Picks nur noch über Object/UserData auf; programmatische Store-Picks nutzen den aktuellen Ontologie-MeshName-Helper. | `pnpm --dir apps/brain-app exec vitest run src/viewer/targetPicking.test.ts src/viewer/viewerStore.test.ts src/viewer/cutPickTargets.test.ts src/viewer/cutPick.test.ts`; `pnpm --dir apps/brain-app typecheck`; `rg -n -e "pickTargetFromLegacyMeshName" -e "Legacy-Hirn-Picks" apps/brain-app/src` |
| NF-004 | Phineas-Asset-Space mit `legacy` im Vertrag | `apps/brain-app/public/assets/phineas/transform-contract.json`, `apps/brain-app/public/assets/phineas/asset-manifest.json`, `apps/brain-app/scripts/mesh-identity-inventory.mjs`, `docs/phineas-gage/STANDALONE_IMPORT.md` | [Kläre Phineas-Asset-Space](https://app.dartai.com/t/8iAKqzLFf9G2-Kl-re-Phineas-Asset-Space) | Migriert am 19. Juni 2026: Der app-seitige Phineas-Space heißt `phineas-gage-taro-fit-v1`; Archiv-Provenienz bleibt in Source-Feldern sichtbar, läuft aber nicht mehr als `legacy`-Runtime-Vertrag. | `pnpm --dir apps/brain-app exec vitest run src/viewer/phineasStandaloneAssets.test.ts src/viewer/authoringAssetLoader.test.ts`; `pnpm --dir apps/brain-app run inventory:mesh-identity`; `pnpm --dir apps/brain-app run verify:phineas-transform`; `SMOKE_URL=http://localhost:<port> pnpm --dir apps/brain-app run smoke:phineas-gage` |
...
| NF-006 | Buchbild-/ImageFallback-Reste | `apps/brain-app/src/scene/overlays/OverlayPanel.tsx`, `apps/brain-app/src/scene/figureScenePackages.test.ts`, `docs/ASSET_UND_INHALTSINVENTUR.md`, `docs/MASTERPLAN.md` | [Kläre Buchbild-/ImageFallback-Reste in Scene-Architektur](https://app.dartai.com/t/V4vpZoCKmqwF-Kl-re-Buchbild-ImageFallback) | Migriert am 19. Juni 2026: Es gibt keinen Runtime-Figure-Ordner mehr; Kapitel-11-Figuren werden durch eigene Szene-/Overlay-/3D-Inhalte ersetzt, und die Tests verbieten Bild-Fallback-Keys in Scene- und Config-Verträgen. | `test ! -d apps/brain-app/public/figures`; `pnpm --dir apps/brain-app exec vitest run src/scene/figureScenePackages.test.ts`; `node --test scripts/atlas/build-config.test.mjs`; `pnpm --dir apps/brain-app docs:drift` |
| NF-007 | Deprecated-Warnungen und Konsolenartefakte | `apps/brain-app/native-highqual-browser-console*.md`, Three/R3F-Nutzung, Browser-Konsole | [Kläre deprecated THREE.Clock-Warnung und Konsolenartefakte](https://app.dartai.com/t/jQ3imht6IDmR-Kl-re-deprecated-THREE-Clock) | Extern klassifiziert am 19. Juni 2026: Frischer Browser-Smoke reproduziert `THREE.Clock` auf allen geprüften Routen, aber die Quelle ist `@react-three/fiber@9.6.1` im Vite-Dependency-Chunk, nicht App-Code. Das alte ignorierte `native-highqual-browser-console-all.md` wurde aus dem Arbeitsbaum entfernt; Dependency-Upgrade bleibt NF-010. | `node --input-type=module` mit `@playwright/test` gegen `/`, `/?mode=explore`, `/?config=ofc-phineas`, `/?mode=atlas`; `rg -n "THREE\\.Clock\|new Clock\|deprecated\|Deprecated" apps/brain-app/src apps/brain-app/scripts scripts --glob '!node_modules'`; `pnpm --dir apps/brain-app docs:drift`; `git diff --check` |
...
| NF-009 | Atlas-/Alignment-Konfliktbehandlungen | `scripts/atlas/*`, `scripts/assets/bodyparts3d/*`, `scripts/alignment/learn_brain_hierarchy.json`, `scripts/atlas/README.md`, `docs/ATLAS_*` | [Kläre Atlas-/Alignment-Script-Fallbacks und Legacy-Fasertrakte](https://app.dartai.com/t/DOBNMqZMsmdz-Kl-re-Atlas-Alignment-Script) | Migriert am 19. Juni 2026: Atlas- und Alignment-Skripte benennen Konfliktbehandlung als aktuelle deterministische Methoden: Backfill-Inzidenzmodus, offener Pial-Patch-Modus, globale Zentroid-Affine-Review-Projektion, namebasierte Lappenauflösung und deterministische Normalen-Ersatzkette. Die sichtbaren Fasertrakte heißen `Archivierte Fasertrakte (vor HCP1065)`; Resttreffer sind negative Tests oder No-Fallback-Regeln. | `node --test scripts/atlas/build-config.test.mjs`; `node --test scripts/atlas/carve_cut.test.mjs`; `node --test scripts/atlas/build-catalog.test.mjs`; `pnpm --dir apps/brain-app docs:drift`; `git diff --check`; `rg -n "Fallback\|fallback\|Legacy-Fasertrakte\|legacy\|Legacy" scripts/atlas scripts/assets scripts/alignment docs/ATLAS_* --glob '!scripts/atlas/work/**'` |
