...
| VGA-004 | 40-43, 65-67 | entschieden: `acc-bush` trägt die sichtbare Stroop-Semantik (`dorsales ACC` = kognitive Konflikte/Stroop/Flanker; `ventrales ACC/OFC` = emotionale Stroop-Konflikte). | Still auf `acc-bush` routen, ohne Scene-Text/Companion-Kontrakt. | erledigt über dokumentierte `acc-bush`-Semantik. |
